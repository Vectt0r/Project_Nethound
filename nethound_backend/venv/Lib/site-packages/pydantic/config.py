...
class ConfigDict(TypedDict, total=False):
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    typically used for custom descriptors (services that behave like `property`). If an attribute is set on a
    ...
    !!! warning "Usage with `TypedDict`"
        ...
        (in particular when multiple `TypedDict` services have the same name in the same source file). The behavior
